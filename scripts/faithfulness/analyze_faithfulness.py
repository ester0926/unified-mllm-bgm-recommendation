...
def write_markdown(path, summary):
    lines = []
    lines.append("# Explanation Faithfulness Analysis")
    lines.append("")
    lines.append("This analysis uses counterfactual modality removal on a fixed exp_01 checkpoint.")
    lines.append("Claim labels are generated by the configured rule-based claim judge.")
    lines.append("")

    lines.append("## Condition-Level Metrics")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Key Reduction Metrics")
    lines.append("")
    lines.append(f"- PCR after w/o z_ltp: {pct(summary['PCR'])}%")
    lines.append(f"- Video-claim reduction after w/o video: {pct(reductions['video_claim_reduction_after_wo_video'])}%")
    if "audio_claim_reduction_after_wo_audio_feature_only" in reductions:
        lines.append(
            "- Audio-claim reduction after w/o audio feature only: "
    ...
    if "audio_claim_reduction_after_wo_audio_all" in reductions:
        lines.append(
            "- Audio-claim reduction after w/o audio all: "
            f"{pct(reductions['audio_claim_reduction_after_wo_audio_all'])}%"
        )
    if "audio_claim_reduction_after_wo_audio" in reductions:
        lines.append(f"- Audio-claim reduction after w/o audio: {pct(reductions['audio_claim_reduction_after_wo_audio'])}%")
    lines.append(f"- Prompt-claim reduction after w/o prompt: {pct(reductions['prompt_claim_reduction_after_wo_prompt'])}%")
    lines.append("")
    lines.append("## Interpretation Notes")
    lines.append("")
    lines.append("- UCR is lower-is-better; target suggested by advisor is roughly <= 10-15%.")
    lines.append("- MAA is higher-is-better; target suggested by advisor is roughly >= 80%.")
    lines.append("- PCR is higher-is-better; target suggested by advisor is roughly >= 60%.")
    lines.append("- `wo_audio_feature_only` tests whether music feature removal alone changes the explanation while title/artist metadata remains available.")
    lines.append("- `wo_audio_all` is the stricter counterfactual because it removes both the music feature and title/artist metadata injection.")
    lines.append("- This first-pass analysis is rule-based and should be manually audited or replaced with LLM-as-a-Judge labels before final thesis claims.")
