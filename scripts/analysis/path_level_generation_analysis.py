...
教授指定的生成側指標，本腳本逐一實作：
  ...
  2. 正確偏好主張率      偏好主張中，其屬性可由該影片的 Stage-4 偏好畫像佐證者之比
...
⚠ 判讀注意：exp_04（No-LTP）在輸入端根本沒有偏好向量，其偏好主張**依定義**必然無證據
   支持，這正是對照的用意；不可解讀為「exp_04 特別會亂講」。
...
OUT_DIR     = RESULTS_DIR / "analysis" / "path_level_generation"
...
def load_profiles(logger) -> dict:
    """video_id → 偏好畫像證據文字（summary_text + 所有 salient_facts）。"""
    ...
    with open(PROFILES_JSONL, "r", encoding="utf-8") as f:
        for line in f:
            d = json.loads(line)
            facts = " ".join(str(x.get("fact", "")) for x in d.get("salient_facts", []) or [])
            out[d["music_id"]] = (str(d.get("summary_text", "")) + " " + facts).lower()
...
COUNT_FIELDS = [
    ...
    "n_pref", "n_pref_supported", "n_pref_partial", "n_pref_notfound", "n_pref_uncheckable",
    ...
    for si, row in enumerate(rows):
        ...
        for cid, claim in enumerate(claims, start=1):
            ...
            if ctype == "preference":
                ...
                verification = verify_against_evidence(terms, pref_evidence)
                counts[si, fi[{
...
def metrics_from_sums(s: np.ndarray, n_samples: int) -> dict:
    ...
    return {
        ...
        # 2/3. 正確 / 不存在偏好主張率（分母為可驗證的偏好主張）
        "preference_correct_rate": div(s[fi["n_pref_supported"]], n_pref_checkable),
...
CONTRAST_METRICS = [
    ...
    "preference_correct_rate", "preference_nonexistent_rate",
...
def main():
    ...
    for exp, label in EXPERIMENTS.items():
        m = all_metrics[exp]
        logger.info("[%s %-18s] 偏好引用=%.4f 正確偏好=%.4f 不存在偏好=%.4f "
                    "元資料支持=%.4f UCR(L1)=%.4f",
                    exp, label, m["preference_claim_ratio"], m["preference_correct_rate"],
    ...
    for exp_a, exp_b in CONTRASTS:
        ...
        pt, lo, hi = res["preference_correct_rate"]
        logger.info("[對照] %-17s 正確偏好主張率 Δ=%+.4f [%+.4f, %+.4f] %s",
                    f"{exp_a} - {exp_b}", pt, lo, hi,
                    "顯著" if (lo > 0 or hi < 0) else "不顯著")

    # ---- 輸出 ---------------------------------------------------------------
    metrics_rows = [{"exp": e, "model": EXPERIMENTS[e], **all_metrics[e]} for e in EXPERIMENTS]
    ...
    summary = {
        ...
        "method": {
            ...
            "preference_evidence": "Stage 4 profiles.jsonl（summary_text + salient_facts），"
                                   "依 video_id 對應，測試集 100% 覆蓋",
            ...
            "caveat": "exp_04 無偏好輸入，其偏好主張依定義必然無證據支持；"
                      "此為對照設計本意，不可解讀為該模型特別容易杜撰。",
        },
    }
...
def write_markdown(path, summary):
    ...
    rows = [
        ...
        ("正確偏好主張率", "preference_correct_rate", "pct"),
    ...
    for c in summary["contrasts"]:
        if c["metric"] not in ("preference_claim_ratio", "preference_correct_rate",
