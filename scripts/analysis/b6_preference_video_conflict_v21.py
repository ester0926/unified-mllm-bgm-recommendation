...
OUT_DIR = PROJECT_ROOT / "results" / "analysis" / "b6_conflict_v21"
...
def two_way_cluster_difference(group_a, group_b, personas, n_boot=BOOTSTRAP_N,
                               seed=BOOTSTRAP_SEED, alpha=CI_ALPHA):
    """Prototype x context pigeonhole bootstrap for a difference of cell means."""
    means_a = {p: float(np.mean(v)) for p, v in group_a.items() if v}
    means_b = {p: float(np.mean(v)) for p, v in group_b.items() if v}
    if not means_a or not means_b:
        return (float("nan"),) * 3
    point = float(np.mean(list(means_a.values())) - np.mean(list(means_b.values())))
    prototypes = sorted({personas[p]["prototype_label"] for p in set(means_a) | set(means_b)})
    contexts = sorted({personas[p]["context_cluster"] for p in set(means_a) | set(means_b)})
    rng = np.random.default_rng(seed)
    draws = []
    for _ in range(n_boot):
        sampled_p = rng.choice(prototypes, size=len(prototypes), replace=True)
        sampled_c = rng.choice(contexts, size=len(contexts), replace=True)
        p_weight = {p: int(np.sum(sampled_p == p)) for p in prototypes}
        c_weight = {c: int(np.sum(sampled_c == c)) for c in contexts}

        def weighted(cell_means):
            values, weights = [], []
            for pid, value in cell_means.items():
                weight = (p_weight[personas[pid]["prototype_label"]]
                          * c_weight[personas[pid]["context_cluster"]])
                if weight:
                    values.append(value)
                    weights.append(weight)
            return float(np.average(values, weights=weights)) if weights else None
...
def main():
        ...
        for r in read_csv(path):
            ...
            for key, val in [("persona_fit", pf), ("video_fit", vf), ("diversity", div),
                ...
                if val is not None and not (isinstance(val, float) and np.isnan(val)):
                    ...
                    cases[pid].setdefault(case_key, {})[key] = val
        return acc, cases

    def _attr(tags, v, spec, a, lo, hi):
        from scripts.analysis.b5_persona_metrics_v21 import attr_satisfied
        return attr_satisfied(tags, v, spec, a, lo, hi)

    matched_result = analyse("matched")
    noltp_result = analyse("no_ltp")
    if matched_result is None:
        raise SystemExit("找不到 persona_v2_matched.csv，請先完成 B5 v2 評估。")
    acc_matched, cases_matched = matched_result
    acc_noltp, cases_noltp = noltp_result if noltp_result is not None else (None, None)
    ...
    for grp in GROUP_LABELS:
        ...
        if acc_noltp:
            for m in ["persona_fit", "video_fit", "R@1"]:
                ...
                for pid in sorted(p for p in cases_matched if group_of.get(p) == grp):
                    for case_key in sorted(set(cases_matched[pid]) & set(cases_noltp.get(pid, {}))):
                        a = cases_matched[pid][case_key].get(m)
                        b = cases_noltp[pid][case_key].get(m)
                        if a is not None and b is not None:
                            deltas[pid].append(float(a - b))
                dpt, dlo, dhi = cluster_bootstrap(deltas)
                row[f"matched_minus_noltp_{m}"] = dpt
                row[f"matched_minus_noltp_{m}_ci_low"] = dlo
                row[f"matched_minus_noltp_{m}_ci_high"] = dhi
    ...
    for m in METRICS:
        ...
        if not pa or not pb:
            continue
        point, lo, hi = two_way_cluster_difference(a, b, personas)
    ...
    # ---- 4b. No-LTP 調整後的交互作用（difference-in-differences）----------
    interaction_rows = []
    if cases_noltp is not None:
        for m in ["persona_fit", "video_fit", "R@1", "MRR"]:
            by_group = defaultdict(lambda: defaultdict(list))
            for pid in sorted(set(cases_matched) & set(cases_noltp)):
                grp = group_of[pid]
                for case_key in sorted(set(cases_matched[pid]) & set(cases_noltp[pid])):
                    a = cases_matched[pid][case_key].get(m)
                    b = cases_noltp[pid][case_key].get(m)
                    if a is not None and b is not None:
                        by_group[grp][pid].append(float(a - b))
            aligned = by_group["高度一致"]
            conflict = by_group["明顯衝突"]
            pa, pb = sorted(aligned), sorted(conflict)
            if not pa or not pb:
                continue
            point, lo, hi = two_way_cluster_difference(aligned, conflict, personas)
            interaction_rows.append({
                "metric": m,
                "contrast": "(Matched-NoLTP)_高度一致 - (Matched-NoLTP)_明顯衝突",
                "difference_in_differences": float(point),
                "ci_low": float(lo), "ci_high": float(hi),
                "significant": bool(lo > 0 or hi < 0),
            })
            print(f"[DiD] {m:12s} = {point:+.4f} [{lo:+.4f},{hi:+.4f}]"
                  f"{' significant' if (lo > 0 or hi < 0) else ' n.s.'}")
    ...
    dump("conflict_interactions.csv", interaction_rows)
    ...
    summary = {
        ...
        "contrasts": contrasts, "interactions": interaction_rows,
        "by_prototype": proto_rows,
        "notes": [
            ...
            "相容度分組與 Persona-fit／Video-fit 共用 Persona 與 GT 標籤，"
            "raw 組間差異只作描述；主要判讀採 Matched-NoLTP 的交互作用。",
            "組間差異與交互作用的 95% CI 採原型 × 情境雙向叢集拔靴；"
            "六種 prototype 與四種 context 分別重抽，再以交叉權重計算。",
        ],
    }
    ...
    L.append("| 指標 | (Matched-NoLTP) 高度一致－明顯衝突 | 95% CI | 顯著 |")
    L.append("|---|---|---|---|")
    for c in interaction_rows:
        L.append(f"| {c['metric']} | {c['difference_in_differences']*100:+.2f}pp | "
                 f"[{c['ci_low']*100:+.2f}, {c['ci_high']*100:+.2f}] | "
                 f"{'是' if c['significant'] else '否'} |")
