...
相對於第一版（b5_persona_metrics.py）新增：
  ...
  • 指標⑧ PCRR Persona Claim Contradiction Rate ← 同上
...
教授 §七 的八項指標對照：
  ...
  ⑧ Persona Claim Contradiction Rate ✔（不再誤稱為 unsupported）
...
【指標定義】
  相關性 rel(track) = 該曲目滿足此 Persona **可操作化**屬性的比例（0–1 分級）
    ...
    nDCG@K         = 以 rel 為分級相關性，IDCG 由同一 500-pool 全候選計算
  ⑦ 說明支持率 = 說明文字中提到的屬性詞，與該 Persona 規格**一致**者之比例
  ⑧ PCRR       = 說明文字中提到的屬性詞，與該 Persona 規格**矛盾**者之比例
...
from scripts.analysis.b5_build_persona_ltp import flip_spec


EVAL_DIR = PROJECT_ROOT / "results" / "main_eval" / "exp_01" / "persona_eval_v21"
OUT_DIR = PROJECT_ROOT / "results" / "analysis" / "b5_personas_v21"
...
def ndcg_at_k(rels, pool_rels, k):
    """標準 nDCG@K：DCG 用模型 top-K，IDCG 用同一 500-pool 的理想 top-K。"""
    r = [0.0 if x is None else x for x in rels[:k]]
    if not r:
        return None
    disc = 1.0 / np.log2(np.arange(2, len(r) + 2))
    dcg = float(np.sum(np.array(r) * disc))
    ideal = sorted((0.0 if x is None else x for x in pool_rels), reverse=True)[:len(r)]
    idcg = float(np.sum(np.array(ideal) * disc))
    return dcg / idcg if idcg > 0 else None


def list_diversity(music_ids, md):
    tagsets = [SPEC.track_tags(md[m]) for m in music_ids if m in md]
    if len(tagsets) < 2:
        return None
    sims = []
    for i in range(len(tagsets)):
        for j in range(i + 1, len(tagsets)):
            union = tagsets[i] | tagsets[j]
            sims.append(len(tagsets[i] & tagsets[j]) / len(union) if union else 0.0)
...
def main():
    ...
    cf_specs = {
        cond: {pid: flip_spec(spec, cond) for pid, spec in pspec.items()}
        for cond in CF_TO_ATTR
    }
    ...
    per_case_attr = {}          # cond -> pid -> sample_idx -> target/original attr dict
    ...
    for cond, rows in data.items():
        ...
        per_case_attr[cond] = defaultdict(dict)
        ranks = []
        ...
        for r in rows:
            ...
            if spec is None:
                continue
            target_spec = cf_specs.get(cond, {}).get(pid, spec)
            ranks.append(int(r["rank"]))

            topk = [x[:11] for x in (r.get("top10_pair_keys") or "").split(";") if x]
            if not topk:
                topk = [r["top1_pair_key"][:11]]
            rels = [relevance(m, target_spec, md, views, p25, p75) for m in topk]
            pool_ids = [x[:11] for x in (r.get("pool_pair_keys") or "").split(";") if x]
            if not pool_ids:
                raise ValueError(f"{cond}/{pid} 缺少 pool_pair_keys，無法計算標準 nDCG")
            pool_rels = [relevance(m, target_spec, md, views, p25, p75) for m in pool_ids]
            ...
            if entry is not None:
                tags = SPEC.track_tags(entry)
                case_key = str(r.get("sample_idx", r.get("video_id", "")))
                per_case_attr[cond][pid][case_key] = {
                    "target": {a: attr_satisfied(tags, views.get(topk[0]), target_spec,
                                                  a, p25, p75) for a in ATTRS},
                    "original": {a: attr_satisfied(tags, views.get(topk[0]), spec,
                                                    a, p25, p75) for a in ATTRS},
                    "diversity": list_diversity(topk[:10], md),
                }
        ...
        if cond in GEN_CONDITIONS:
            ...
            expl_rows.append({
                ...
                "PCRR": c_pt, "PCRR_ci_low": c_lo, "PCRR_ci_high": c_hi,
                "share_with_any_attribute_claim": r_pt,
            })
            print(f"    ⑦支持率={s_pt:.4f} [{s_lo:.4f},{s_hi:.4f}]  "
                  f"⑧PCRR={c_pt:.4f} [{c_lo:.4f},{c_hi:.4f}]  "
                  f"有屬性主張的比例={r_pt:.4f}")
    ...
    def mean_acr(cond, pid):
        cases = per_case_attr[cond][pid].values()
        vals = []
        for c in cases:
            v = [x for x in c["target"].values() if x is not None]
            if v:
    ...
    if expl_by_persona:
        for a, b in [("matched", "shuffled"), ("matched", "no_ltp"),
            ...
            if a not in expl_by_persona or b not in expl_by_persona:
                continue
            for metric, idx in [("support_rate", 0), ("PCRR", 1)]:
                diffs = {}
    ...
    for cond, attr in CF_TO_ATTR.items():
        ...
        diversity_delta_by_p = defaultdict(list)
        for pid in sorted(set(per_case_attr[cond]) & set(per_case_attr["matched"])):
            mc, cc = per_case_attr["matched"][pid], per_case_attr[cond][pid]
            for case_key in sorted(set(mc) & set(cc)):
                if attr is not None:
                    mv = mc[case_key]["original"].get(attr)
                    cv_target = cc[case_key]["target"].get(attr)
                    if mv is not None and cv_target is not None:
                        cda_by_p[pid].append(float(bool(mv) and bool(cv_target)))
                else:
                    mdv, cdv = mc[case_key]["diversity"], cc[case_key]["diversity"]
                    if mdv is not None and cdv is not None:
                        delta = cdv - mdv
                        diversity_delta_by_p[pid].append(delta)
                        expect_more_diverse = pspec[pid]["consistency"] in ("very_high", "high")
                        cda_by_p[pid].append(float(delta > 0 if expect_more_diverse else delta < 0))
                ch = [float(mc[case_key]["original"][a] !=
                            cc[case_key]["original"][a]) for a in ATTRS
                      if a != attr and mc[case_key]["original"][a] is not None
                      and cc[case_key]["original"][a] is not None]
                if ch:
        ...
        div_delta = cluster_bootstrap(diversity_delta_by_p) if diversity_delta_by_p else \
            (float("nan"),) * 3
        cda_rows.append({"counterfactual": cond,
                         "target_attribute": attr or "consistency/diversity",
                         "CDA": cda[0], "CDA_ci_low": cda[1], "CDA_ci_high": cda[2],
                         "IAD": iad[0], "IAD_ci_low": iad[1], "IAD_ci_high": iad[2],
                         "diversity_delta": div_delta[0],
                         "diversity_delta_ci_low": div_delta[1],
                         "diversity_delta_ci_high": div_delta[2]})
    ...
    dump("persona_metrics_v21_by_condition.csv", by_cond)
    dump("persona_metrics_v21_gaps.csv", gaps)
    dump("persona_metrics_v21_counterfactual.csv", cda_rows)
    dump("persona_metrics_v21_explanation.csv", expl_rows)
    dump("persona_metrics_v21_explanation_gaps.csv", expl_gaps)
    ...
    summary = {
        ...
        "notes": [
            ...
            "⑦與 PCRR 為規則式詞彙比對，僅反映詞彙層級一致／矛盾；"
            "No-LTP 條件提供『無偏好輸入時仍會說出的屬性詞』機率基線。",
            "情緒價向不可操作化，未納入任何指標。",
            "統計以 Persona 為叢集重抽單位（同一 Persona 的 20 支查詢彼此相關）。",
        ],
    }
    (OUT_DIR / "persona_metrics_v21_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    L = ["# B5 Persona 偏好可控性完整指標（v21）\n",
    ...
    for n in summary["notes"]:
        L.append(f"- {n}")
    (OUT_DIR / "persona_metrics_v21_summary.md").write_text("\n".join(L) + "\n", encoding="utf-8")
