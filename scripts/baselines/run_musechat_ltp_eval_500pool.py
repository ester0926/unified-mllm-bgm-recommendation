...
"""
musechat_ltp（MVTFusionWithLTP）完整 500-pool 評估腳本。

【實驗定位】
musechat_ltp 使用 MuseChat-light 架構（非統一 MLLM）+ LTP 輸入，相當於 baseline 對照，
用於 2×2 析因（unified-architecture × LTP）分析。
exp_01~07 才是統一 MLLM 的消融實驗系列。

這是 musechat_ltp 論文最終數字的來源，與以下腳本使用完全相同的評估協定：
...
2×2 比較對照：
  ...
  │              │   無 LTP         │   有 LTP            │
  ├──────────────┼──────────────────┼─────────────────────┤
  │ Non-unified  │ MuseChat-light   │ musechat_ltp（本腳本）│
...
輸出：
    checkpoints/exp_08_ltp/detailed_eval/
      musechat_ltp_500pool_ranking_samples.csv   ← Wilcoxon 檢定輸入
      musechat_ltp_500pool_summary.json          ← 指標摘要

    ⚠️ 注意：OUTPUT_DIR 指向 checkpoints/exp_08_ltp/（訓練 checkpoint 所在的實體資料夾，
    資料夾名稱 exp_08_ltp 為歷史命名，不更動避免破壞已訓練模型路徑）。
"""
...
# musechat_ltp checkpoint（訓練完成後 mvt_ltp_best.pt 的路徑）
# 注意：資料夾名稱 exp_08_ltp 為歷史命名（實體訓練 checkpoint 所在位置），不更動。
...
# 注意：OUTPUT_DIR 指向實體資料夾 exp_08_ltp（歷史命名），不更動。
OUTPUT_DIR    = os.path.join(core.BASE_DIR, "checkpoints", "exp_08_ltp", "detailed_eval")
OUTPUT_PREFIX = f"musechat_ltp_{POOL_SIZE}pool"
...
def setup_logger():
    ...
    logger = logging.getLogger("musechat_ltp_eval")
...
def load_musechat_ltp_model(muse_cfg_mod, mvt_ltp_mod, device, logger):
    """載入 musechat_ltp MVTFusionWithLTP checkpoint。"""
    if not os.path.isfile(MVT_LTP_CKPT_PATH):
        raise FileNotFoundError(
            f"找不到 musechat_ltp checkpoint：{MVT_LTP_CKPT_PATH}\n"
    ...
    model.eval()
    logger.info("musechat_ltp model loaded: %s", MVT_LTP_CKPT_PATH)
    return model
...
def run_ranking(model, ltp_dict, test_pairs, target_bank, song_ids, logger):
    ...
    try:
        for idx in tqdm(range(n_eval), desc=f"musechat_ltp ranking ({POOL_SIZE}-pool)"):
    ...
    logger.info(
        "musechat_ltp ranking: R@1=%.4f  R@5=%.4f  R@10=%.4f  MedR=%.1f  (n=%d)",
...
def load_generator(gen_mod, muse_cfg_mod, device, logger):
    ...
    if os.path.isdir(lora_dir) and hasattr(generator.llm, "load_adapter"):
        generator.llm.load_adapter(lora_dir, adapter_name="musechat_ltp_gen", is_trainable=False)
        if hasattr(generator.llm, "set_adapter"): generator.llm.set_adapter("musechat_ltp_gen")
    generator.eval()
...
def run_generation(generator, ranking_rows, song_bank_tensor, song_ids,
    ...
    rows        = []
    from tqdm import tqdm
    for rank_row in tqdm(ranking_rows, desc=f"musechat_ltp {mode} generation"):
...
def main():
    ...
    logger.info("=" * 65)
    logger.info("musechat_ltp（MVTFusionWithLTP）500-pool 完整評估")
    ...
    # ── 2. 載入 musechat_ltp 模型 + LTP ──────────────────────────────────────
    muse_cfg_mod, mvt_ltp_mod, gen_mod = load_musechat_modules()
    model    = load_musechat_ltp_model(muse_cfg_mod, mvt_ltp_mod, device, logger)
    ...
    # ── 7. 印出結果 ───────────────────────────────────────────────────────────
    print("\n" + "=" * 65)
    print(f"musechat_ltp（MVTFusionWithLTP）500-pool 評估結果")
    ...
    print("【2×2 比較對照（更新後）】")
    print("              │ 無 LTP          │ 有 LTP")
    print("  Non-unified │ MuseChat-light  │ musechat_ltp（本結果）")
